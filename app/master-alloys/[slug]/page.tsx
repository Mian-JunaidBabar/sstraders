...
export async function generateMetadata({
  ...
  const product = masterAlloys.find((p) => p.slug === slug);
  if (!product) return {};
  return buildProductMetadata(product, "master-alloys");
}

export default async function MasterAlloyProductPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const productSchema = getProductSchema(product, "master-alloys");
  const breadcrumbSchema = getBreadcrumbSchema([
    { name: "Home", url: "/" },
    { name: "Master Alloys", url: "/master-alloys" },
    { name: product.name, url: `/master-alloys/${product.slug}` },
  ]);
